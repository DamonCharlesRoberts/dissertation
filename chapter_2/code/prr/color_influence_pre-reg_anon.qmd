...
I randomly assign participants to three conditions. The conditions prompt subjects to "Imagine that [they] are driving along a road and see this yard sign" with the same message "Vote for Riley." The conditions vary on the color of the background for the image.[^2] In the control condition, the background was white. Then I had a red yard sign and a blue yard sign condition. I asked participants questions operationalized as outcomes of interest on a separate screen.

[^2]: The Appendix contains the images used for the treatments and the particular wording for the dependent variables.
    ...
    PartyGuessTrialThree = draw_ordered( # Define PartyGuessTrialTwo outcome variable
        x = rnorm(
            ...
            mean = (
              ...
              - 0.05 * RedTreatment * age + 0.05 * BlueTreatment * age 
              + 0.1 * Attention + 0.1 * Knowledge + E
            )
        ),
        breaks = c(
...
\begin{split}
  \alpha \sim Student-T(3,0,2.5) \\
  \beta_i \sim Normal(0, 1) \\
  \hat{PartyGuess_i} = Cumulative(logit^{-1}(\alpha_1 + \alpha_2 + \beta_1 \times Red Treatment + \beta_2 \times Blue Treatment + \beta_3 \times age + \beta_4 \times Red Treatment \times age + \beta_5 \times Blue Treatment \times age + \beta_6 \times Attention + \beta_7 \times Knowledge))
\end{split}
$${#eq-party-guess-priors}

These priors suggest that I believe 68% of my beta coefficients, given the data, should be between -1 and 1; with the bulk of the estimates at 0. 

I run this model on the 2000 total samples. While doing so, I take the mean value of the posterior draws for each coefficient and subtract the parameter value for that coefficient. @fig-discrepancies presents a boxplot displaying these discrepancies.

```{r}
#| label: fig-discrepancies
#| fig-cap: Discrepancies between mean posterior draws and parameters

Sample200Results <- readRDS("../../data/prr/sample200SimResults.RData")
Sample400Results <- readRDS("../../data/prr/sample400SimResults.RData")
Sample600Results <- readRDS("../../data/prr/sample600SimResults.RData")
Sample800Results <- readRDS("../../data/prr/sample800SimResults.RData")

RedTreatmentPlot <- ggplot() +
  geom_boxplot(
    aes(
      x = "n = 200",
      y = MeanRed),
    data = Sample200Results
  ) +
  geom_boxplot(
    aes(
      x = "n = 400",
  ...
  geom_boxplot(
    ...
    data = Sample800Results
  ) +
  theme_minimal()

BlueTreatmentPlot <- ggplot() +
  geom_boxplot(
    aes(
      x = "n = 200",
      y = MeanBlue
...
To examine whether the use of colors on yard signs varies systematically depending on the electoral context, using the `selenium` [@selenium] and `time` [@time] `python` [@python] packages, I collect images from 2018, 2020, and 2022 Congressional elections for the House of Representatives across the United States. These yard signs are pulled together on one website by the Center for American Politics and Design[^10]. With these data, I detect the percentage of the "Republican Red" and "Democratic Blue" on the yard signs. I then use redistricting data from the [MIT election lab and Redistricting Data Hub](https://redistrictingdatahub.org/data/about-our-data/) to determine districts that did and did not have changes in the 5-year rolling average partisan vote share but that are also nationally representative. As there are a number of external factors that may effect choice in yard sign color besides perceptions of how it may attract voters, this threatens my ability to make claims about any estimation of an Average Treatment Effect [^11]. Instead these data are used to perform a synthetic control design to give me causal leverage over the role that electoral context has on the proportion of partisan colors (i.e., red and blue) on yard signs relative to non-partisan colors by estimating a Local Average Treatment Effect [^12] on districts that were redistricted but vary in whether the redistricting leads to a partisan gerrymandered district or not. This analysis aims to bear evidence on the hypothesis that campaigns respond to the preferences of partisan voters and adjust their branding as a result. In this case, the branding is the color of the yard sign. First, I'll explain how I collected data on the proportion of partisan and non-partisan colors on these yard signs from the [Center for American Politics and Design](https://www.politicsanddesign.com).

[^10]: See: https://www.politicsanddesign.com/

[^11]:  $\frac{1}{n}\sum{}{i}(Y_i(d=1) - Y_i(d=0))$ [see @lundberg_et-al_2021_asr]

[^12]: $E[(Y^1_i - Y^0_i) | d^1_i - d^0_i = 1]$ \[see @cunningham_2021_yup\]

I collected the GOP logo used on their official Twitter account during the 2022 midterm election cycle to provide an example of how color detection works. With the `opencv` package [@opencv_library], I load this image and convert it to a three-dimensional `numpy` [@numpy] array that contains information about the GBR (reversed RBG) values for the pixels in that image. I resized the images to be a standardized 224 $x$ 224 pixels. I train the computer to detect a range of GBR values that encompass the official "Republican Red" [^13]. For the broader exercise, I do it for the color white[^14] and "Democratic blue" [^15]. Once this range of values is specified, the computer detects the pixels that do not contain values within this pre-specified range and converts those values to represent the color black. @fig-color-detection-example presents this process using the `opencv` [@opencv_library] and `matplotlib` [@matplotlib] packages.

[^13]: lower values: (93, 9, 12), higher values: (236, 69, 75)

[^14]: upper and lower values: (255, 255, 255)

[^15]: lower values: (0, 18, 26), higher values: (102, 212, 255)
